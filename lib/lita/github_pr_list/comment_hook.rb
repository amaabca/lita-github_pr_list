module Lita
  module GithubPrList
    class CommentHook
      attr_accessor :request, :response, :payload, :commenter, :issue_owner, :issue_title, :issue_body, :status, :issue_html_url

      def initialize(params)
        self.response = params.fetch(:response, nil)
        self.request = params.fetch(:request, nil)

        raise 'invalid params' if response.nil? || request.nil?

        # https://developer.github.com/v3/activity/events/types/#issuecommentevent
        self.payload = JSON.parse(request.body.read)
        self.commenter = payload["sender"]["login"]
        self.issue_owner = payload["issue"]["user"]["login"]
